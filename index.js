...
function epochToJsDate(epochTime){
  ...
  const setupUI = (user) => {
    if (user) {
      ...
      var dbPath = 'sensorData/';
      var chartPath = 'sensordata/';
      ...
      chartRef.on('value', snapshot =>{
        ...
        dbRef.orderByKey().limitToLast(chartRange).on('child_added', snapshot =>{
          ...
          var jsonData = snapshot.toJSON(); // example: {temperature: 25.02, TDS 560, pH 7.1, timestamp:1641317355}
          ...
          plotValues(chartTD, timestamp, TDS);
          plotValues(chartP, timestamp, pH);
        });
      });
         console.log("after plotting function")
